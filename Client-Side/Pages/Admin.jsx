...
export default function Admin(props) {
  ...
  const handleApprovedRestaurant = async (id, email) => {
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        { text: 'Add', style: 'destructive', onPress: () => { changeApprovedRestaurant(id, email); },
       }
      ],
      { cancelable: true }
    );
  ...
  const renderRestaurantItem = ({ item }) => {
    ...
    return (
      ...
      if (selectedOption === 'pending' && item.approved != "true") {
        return (
          <View style={{ flexDirection: 'row', alignItems: 'center', paddingVertical: 8 }}>
          <Image source={{ uri: item.image }} style={{ width: 50, height: 50, borderRadius: 25, margin: 10 }} />
          ...
          <TouchableOpacity onPress={() => handleApprovedRestaurant(item._id, item.email)}>
